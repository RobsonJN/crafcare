    ...
    ['feminino', 'masculino', 'body'].forEach(genero => {
        ...
        visibleCards.sort((a, b) => {
            ...
            if (sortBy === 'price-desc') {
                ...
                priceA = parseFloat(a.dataset.maxPrice);
                priceB = parseFloat(b.dataset.maxPrice);
            } else if (sortBy === 'price-asc') {
                // Para menor preço, usar o preço mínimo
                priceA = parseFloat(a.dataset.minPrice);
                priceB = parseFloat(b.dataset.minPrice);
            } else {
                priceA = parseFloat(a.dataset.minPrice);
                priceB = parseFloat(b.dataset.minPrice);
            }
...
function saveDeliveryInfo() {
    ...
    if (deliveryType === 'delivery') {
        ...
        info = { ...info, name, address, complement };
    } else {
...
function sendToWhatsApp() {
    ...
    if (deliveryInfo.type === 'pickup' && !deliveryInfo.name) {
        openDeliveryModal();
        document.querySelector('.delivery-option[data-type="pickup"]').click();
        alert('Para retirada no local, é necessário informar seu nome.');
        return;
    }
    
    // Validação específica para entrega
    if (deliveryInfo.type === 'delivery' && (!deliveryInfo.name || !deliveryInfo.address)) {
        openDeliveryModal();
        document.querySelector('.delivery-option[data-type="delivery"]').click();
        alert('Para entrega, é necessário informar seu nome e endereço.');
        return;
    }
